...
def move(grid, current_position, visited_array)
    current_value = grid[current_position[0]][current_position[1]]
    if current_value == '^'
        return false if (current_position[0] - 1) < 0
        ...
        if next_value == '#'
            ...
            return 1 if visited_array[arr_key] && visited_array[arr_key] > 1
        ...
        return false if current_position[1] + 1 > grid[0].length
        ...
        return false if (current_position[0] + 1) >= grid.length
        next_value = grid[current_position[0] + 1][current_position[1]]
        ...
        return false if (current_position[1] - 1) < 0
        next_value = grid[current_position[0]][current_position[1] - 1]
    ...
    move(grid, current_position, visited_array)
end
...
initial_grid.each_with_index do |y, y_idx|
    if y_idx >= initial_position[0]
        puts "Y INDEX #{y_idx}"
        y.each_with_index do |x, x_idx|
            next if x_idx == initial_position[1] && y_idx == initial_position[0]
            modified_grid = initial_grid.map(&:dup)
            modified_grid[y_idx][x_idx] = '#'
            result = move(modified_grid, initial_position, {})
            final_obstacle_count += 1 if result == 1
        end
    end
end
